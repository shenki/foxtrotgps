...
if len(sys.argv) <3:
  print "usage:"
  print "./osb2tango gpxfile poidbfile"
  ...
  if wpt.tag=="{http://www.topografix.com/GPX/1/1}wpt":
    ...
    for sub in wpt:
      if sub.tag=="{http://www.topografix.com/GPX/1/1}desc":
        data['desc']=sub.text
    # build the query
    idmd5 = hashlib.md5(data['lon']+data['lat']).hexdigest()[0:18] #not the format tangogps uses
    query = "insert into poi (idmd5, lon, lat, keywords, visibility, cat, subcat, price_range, extended_open, desc) values ('"+idmd5+"', '"+data['lon']+"', '"+data['lat']+"', 'OpenStreetBug', 0.0, 14.0, 2.0, 3.0, 0.0, '"+escapeQuote(data['desc'])+"')" #tangogps crashes if some of these remain NULL.
    cur.execute(query)
